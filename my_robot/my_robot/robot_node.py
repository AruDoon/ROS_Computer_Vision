...
class ChaseTargetNode(Node):
    def __init__(self):
        ...
        self.yaw = 0.0  # Current orientation from IMU in radians
        ...
        # Control parameters - TUNED FOR BETTER FOLLOWING
        ...
        self.heading_tolerance = 0.6  # Radians (~17 degrees)
    ...
    def dilate_obstacles(self, grid_map):
        ...
        try:
            ...
            self.get_logger().info(f"Obstacle dilation: {obstacle_count_original} -> {obstacle_count_dilated} cells "
                                 f"(clearance: {self.obstacle_clearance}m, kernel: {kernel_size}x{kernel_size})")
    ...
    def image_callback(self, msg):
        ...
        try:
            ...
            # Find target (largest red contour)
            target_detected = False
            target_contours, _ = cv2.findContours(red_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
            if target_contours:
                ...
                if cv2.contourArea(target_contour) > 50:
                    ...
                    if M['m00'] != 0:
                        ...
                        if self.is_valid_position(new_target_pos):
                            ...
                            # Draw target
                            cv2.drawContours(cv_image, [target_contour], -1, (255, 0, 0), 2)
                            cv2.circle(cv_image, (cx, cy), 5, (255, 0, 0), -1)
                            cv2.putText(cv_image, f"Target ({new_target_pos[0]:.1f},{new_target_pos[1]:.1f})", 
            ...
            # Draw obstacles
            obstacle_contours, _ = cv2.findContours(yellow_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
            ...
            cv2.putText(cv_image, f"Yaw: {math.degrees(self.yaw):.1f}°", 
            ...
            try:
                ...
                obstacle_pixels = np.where(yellow_mask > 0)
    ...
    def imu_callback(self, msg):
        ...
        try:
            q = msg.orientation
            if abs(q.w) > 1 or abs(q.x) > 1 or abs(q.y) > 1 or abs(q.z) > 1:
                return
                
            siny_cosp = 2.0 * (q.w * q.z + q.x * q.y)
            cosy_cosp = 1.0 - 2.0 * (q.y**2 + q.z**2)
            self.yaw = math.atan2(siny_cosp, cosy_cosp)  # Keep in radians
        except Exception as e:
            self.get_logger().error(f"IMU processing failed: {e}")
    ...
    def planning_callback(self):
        """Plan the path from robot to target."""
        try:
            if (self.robot_pos is None or self.target_pos is None or 
                self.dilated_grid_map is None or self.detection_confidence < 3):
                return
            ...
            if new_path and len(new_path) > 1:
                self.path = new_path
                self.current_waypoint_index = 0  # Reset to start of new path
                self.get_logger().info(f"New path planned with {len(self.path)} waypoints (clearance: {self.obstacle_clearance}m)")
            else:
                self.get_logger().warn("No path found to target with current clearance")
    ...
    def control_callback(self):
        """IMPROVED: Control the robot to follow the path step by step."""
        try:
            ...
            heading_error = desired_yaw - self.yaw
            ...
            twist.angular.z = max(-max_angular_vel, 
                                min(max_angular_vel, self.angular_gain * heading_error))
